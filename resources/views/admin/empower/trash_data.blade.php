        ...
        <div class="card mb-4">
            <div class="card-body">
                ...
                <div class="table-responsive">
                    <a href="{{route('empower-list')}}" class="btn btn-primary btn-sm" style="float:right">Previous</a>
                    <br>
                        ...
                        <tbody>
                            @if (count($trashData)>0)
                                @foreach ($trashData as $key => $cms)
                                    <tr>
                                        <td>{{$key+1}}</td>
                                        <td>{!! Str::limit($cms->description, 60, '...') !!}</td>
                ...
                function(e) {
                    if (e == true) {
                        ...
                        $.ajax({
                            type: "POST",
                            dataType: "json",
                            url: "{{ route('empower-restore-data') }}",
                            data: {
                                "_token": "{{ csrf_token() }}",
                                'id': id,
                                'table_name': table
