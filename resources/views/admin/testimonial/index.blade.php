        ...
        <div class="card mb-4">
            <div class="card-body">
                ...
                <div class="table-responsive">
                    <a href="{{route('testimonial-trash')}}" class="btn btn-warning btn-sm" style="float:right"><i class="fa fa-trash"></i> Trash</a>
                    <a href="{{route('testimonial-create')}}" class="btn btn-primary btn-sm" style="float:right"><i class="fa fa-plus"></i>Add Testimonial</a>
                    <br>
                        ...
                        <tbody>
                            @foreach ($testimonial as $key => $item)
                                <tr>
                                    ...
                                    <td>
                                        @if($item->status==0)
                                        <span data-dc="{{base64_encode($item->id)}}" class="badge badge-danger">Deactive</span>
                                        <span data-ac="{{base64_encode($item->id)}}" class="badge badge-success d-none">Active</span>
                                    @else
                                        <span data-dc="{{base64_encode($item->id)}}" class="badge badge-danger d-none">Deactive</span>
                                        <span data-ac="{{base64_encode($item->id)}}" class="badge badge-success">Active</span>
                                    @endif
                                    </td>
                                    <td>
                                        <a href="{{url('admin/testimonial/edit/'.base64_encode($item->id))}}">
                                            <button class="btn btn-primary btn-sm" type="button"> <i class='fa fa-edit'></i> Edit</button>
                                        </a>
                                        <button class="btn btn-danger btn-sm deleteBtn" type="button" data-id="{{ base64_encode($item->id) }}" > <i class='fa fa-trash'></i> Delete</button>
                                        @if($item->status==1)
